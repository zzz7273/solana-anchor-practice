...
describe("counter-pda", () => {
  ...
  // 直接使用test框架默认的钱包，可能有一些默认行为不知道如何控制。后面交易时还是指定自定义钱包来签名吧
  ...
  const program = anchor.workspace.counter_pda as Program<CounterPda>;
  ...
  it("Is create counter!", async () => {
    ...
    await airdropIfRequired(anchor.getProvider().connection, keypair1.publicKey, 2*anchor.web3.LAMPORTS_PER_SOL, 1*anchor.web3.LAMPORTS_PER_SOL);
    await airdropIfRequired(anchor.getProvider().connection, keypair2.publicKey, 2*anchor.web3.LAMPORTS_PER_SOL, 1*anchor.web3.LAMPORTS_PER_SOL);
  ...
  it("Is increament for owner!", async () => {
    ...
    console.log(`counter[${c1.pda}] chain info before increment: `, cc0);
    ...
    expect(cc.count.toNumber()).to.equal(1);
  ...
  it("Is increament for others should be success!", async () => {
    ...
    expect(cc.count.toNumber()).to.equal(2);
  ...
  it("Is set remark!", async () => {
    ...
    const tx = await program.methods
      .setRemark(c1.counterName,sss)
      .accountsPartial({counter: c1.pda,owner: keypair1.publicKey, owner: keypair1.publicKey})
      .signers([keypair1])
      .rpc();
    // console.log("create counter c2 signature", tx);
